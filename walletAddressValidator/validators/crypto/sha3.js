...
Keccak.prototype.update = function (message) {
  ...
  const blocks = this.blocks; const byteCount = this.byteCount; const length = message.length
  const blockCount = this.blockCount; let index = 0; const s = this.s; let i; let code
...
Keccak.prototype.encodeString = function (str) {
  let notString; const type = typeof str
  if (type !== 'string') {
    if (type === 'object') {
      if (str === null) {
        throw ERROR
  ...
  let bytes = 0; const length = str.length
  if (notString) {
    bytes = length
  } else {
...
Keccak.prototype.finalize = function () {
  ...
  this.finalized = true
  const blocks = this.blocks; let i = this.lastByteIndex; const blockCount = this.blockCount; const s = this.s
...
Keccak.prototype.toString = Keccak.prototype.hex = function () {
  ...
  const blockCount = this.blockCount; const s = this.s; const outputBlocks = this.outputBlocks
  const extraBytes = this.extraBytes; let i = 0; let j = 0
  let hex = ''; let block
...
Keccak.prototype.digest = Keccak.prototype.array = function () {
  ...
  const blockCount = this.blockCount; const s = this.s; const outputBlocks = this.outputBlocks
  const extraBytes = this.extraBytes; let i = 0; let j = 0
  const array = []; let offset; let block
  while (j < outputBlocks) {
    for (i = 0; i < blockCount && j < outputBlocks; ++i, ++j) {
      offset = j << 2
      block = s[i]
