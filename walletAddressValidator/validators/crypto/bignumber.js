...
const trim = String.prototype.trim || function () { return this.replace(/^\s+|\s+$/g, '') }
...
function convert (nStr, baseOut, baseIn, sign) {
  let e, dvs, dvd, nArr, fracArr, fracBN
...
P.dividedBy = P.div = function (y, b) {
  ...
  const xe = this.e
  const xs = this.s
  const yc = (id = 2, y = new BigNumber(y, b)).c
  const ye = y.e
  const ys = y.s
  const s = xs == ys ? 1 : -1
...
P.modulo = P.mod = function (y, b) {
  let x = this
  const xc = x.c
  const yc = (id = 9, y = new BigNumber(y, b)).c
  let i = x.s
  let j = y.s
...
P.times = P.mul = function (y, b) {
  ...
  const x = this
  let xc = x.c
  let yc = (id = 11, y = new BigNumber(y, b)).c
  let i = x.e
  let j = y.e
  let a = x.s
...
P.toFraction = P.toFr = function (maxD) {
  let q; let frac; let n0; let d0; let d2; let n; let e
  let n1 = d0 = new BigNumber(ONE)
  let d1 = n0 = new BigNumber('0')
  const x = this
  const xc = x.c
  const exp = MAX_EXP
  const dp = DECIMAL_PLACES
  const rm = ROUNDING_MODE
  let d = new BigNumber(ONE)
